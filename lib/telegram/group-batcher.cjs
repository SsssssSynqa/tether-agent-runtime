...
class GroupConversationBatcher {
  constructor({
    ...
    contextDemotionMs = 0,
    honorStopInstructions = true,
    suppressNonConversationalBotMessages = true,
  }) {
    ...
    this.honorStopInstructions = honorStopInstructions !== false;
    this.suppressNonConversationalBotMessages = suppressNonConversationalBotMessages !== false;
    this.chats = new Map();
  }
  ...
  enqueue(message) {
    ...
    if (
      this.suppressNonConversationalBotMessages
      && message.senderIsBot
      && isNonConversationalBotMessage(message.text)
    ) {
    ...
    if (this.honorStopInstructions && !message.senderIsBot && isStopInstruction(message.text)) {
      state.muted = true;
