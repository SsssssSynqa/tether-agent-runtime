...
const labels = {
  ...
  claims: 'Claims', events: 'Events', projections: 'Projections', reviews: 'Reviews',
}
...
function semantic(data, state) {
  ...
  return `<section><header class="page-head"><div><p class="eyebrow">VERIFIABLE MEMORY</p><h2>Semantic records</h2><p>Claims, events, projections, and reviews remain distinct. A fluent sentence is not evidence by itself.</p></div><div class="segmented">${SEMANTIC_KINDS.map((kind) => `<button data-semantic-kind="${kind}" class="${state.semanticKind === kind ? 'active' : ''}">${labels[kind]} <span>${number(data.semantic.counts?.[kind])}</span></button>`).join('')}</div></header>
